...
import (
	...
	"github.com/renatofm-dev/FC_ChatService/internal/domain/gateway"
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	userMessage, err := entity.NewMassage("user", input.UserMessage, chat.Config.Model)
	if err != nil {
	...
	assistant, err := entity.NewMassage("assistant", fullResponse.String(), chat.Config.Model)
	if err != nil {
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	initialMessage, err := entity.NewMassage("system", input.Config.InitialSystemMessage, model)
	if err != nil {
